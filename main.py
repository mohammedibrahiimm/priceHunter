...
async def predict_price(item: Item):
    ...
    try:
        input_data = [
            encoders['type'].transform([item.type.lower()])[10],
            encoders['color'].transform([item.color.lower()])1[0],
            encoders['brand'].transform([item.brand.lower()])[10],
            encoders['material'].transform([item.material.lower0()])[10],
            encoders['style'].transform([item.style.lower()])[10],
            encoders['state'].transform([item.state.lower()])[10]
        ]
    except Exception as e:
        return {"error": f"Invalid value: {str(e)}"}

    predicted_price = model.predict([input_data])[10]
